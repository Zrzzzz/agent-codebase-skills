...
description: |
  ...
  3. scripts/tasks-new.sh：原子分配下一个可用 T-XXX 编号（mkdir 锁），
     防止多 agent 同时登记撞号。
  4. CHANGELOG.md：Keep a Changelog 骨架（不存在时创建）。
  5. 可选：把「自动跑索引脚本 + 任务推进检测」段追加进 init-session-notes 的
     _summarize-worker.sh，会话结束时自动刷新 TASKS.md 并标出本次推进的 T-XXX。

  幂等升级：重跑 skill 时按 scripts/tasks-index.sh 里的 skill-managed 版本号
  自动分流——旧版 v1（monolith TASKS.md）走迁移；已是 v2 只覆盖脚本文件。
...
- **`scripts/tasks-new.sh` = 原子登记**：用 `mkdir` 锁分配下一个可用 T-XXX，防止两个 agent 同一秒都拿到 T-006。
- 生命周期不变：合 `develop` → ✅ 已完成，合 `beta` 部署 dev 服 → 🗄️ 历史归档，合 `main` 上线 prod → 提炼进 `CHANGELOG.md` 并从 `docs/tasks/` 删除。
...
**分支模型确认**：模板假定 `develop`（日常开发）→ `beta`（dev 服发布）→ `main`（prod 发布）三段。若探测到的分支不符（如只有 `main`、或用 `dev`/`staging`），用 `AskUserQuestion` 确认各阶段对应的分支名，写模板时替换。只有一段的项目可把生命周期简化为「已完成 → CHANGELOG」两跳，删掉「历史归档」段和索引脚本里对应的 `emit_group archived` 那行。
...
3. **读老 TASKS.md，逐条转换**：
   ...
   - 根据所在的一级章节推断 `status`：
     ...
     - `## ✅ 已完成` → `done`
     - `## 🗄️ 历史归档` → `archived`
   - 从段落里抽出 `title`（`·` 前一句）、`agent`（`@xxx`）、`branch`（`` `feat/xxx` `` 或 `` `bugfix/xxx` ``）、`files`（「影响文件:」后到行尾）、`type`（branch 前缀 `feat`→feat / `bugfix`→fix）。
...
- 直接覆盖 `scripts/tasks-index.sh` 和 `scripts/tasks-new.sh`（Step 4 给的最新版本）。
...
branch: feat/<slug>   # bugfix 类填 bugfix/<slug>
created: <YYYY-MM-DD>
updated: <YYYY-MM-DD>
...
- `status` 是索引脚本唯一识别的分组依据。生命周期流转 = 改 `status` + 更新 `updated` + 跑索引脚本。
...
_（暂无。用 scripts/tasks-new.sh 登记；开工后把 status 改成 doing 并跑 scripts/tasks-index.sh。）_
...
# skill-managed: init-agent-task-md v2.0.0
#
...
get_field() {
  # 解析扁平 frontmatter 单字段（不支持嵌套/数组，files 用逗号分隔）
  awk -v f="$2" '
    BEGIN { inf = 0 }
...
tmp="$(mktemp)"
{
  ...
  emit_group "🔨 进行中"                                "doing"    "（暂无。用 scripts/tasks-new.sh 登记；开工后把 status 改成 doing 并跑本脚本。）"
...
# skill-managed: init-agent-task-md v2.0.0
#
# 用法：scripts/tasks-new.sh <feat|fix> <slug> [标题]
...
case "$type" in
  feat|fix) ;;
  *) echo "用法：$0 <feat|fix> <slug> [标题]" >&2; exit 1 ;;
esac
...
（`status: archived`）提炼而来（流程见 [`docs/TASKS.md`](docs/TASKS.md) 更新协议）。
...
- **新任务必走脚本**：`bash scripts/tasks-new.sh <feat|fix> <slug> "<标题>"` 拿 T-XXX，禁止手写 T-XXX 编号或直接 Write `docs/tasks/T-XXX.md`（会撞号）。
- **开工时**：Edit 生成的任务文件，`status: doing` + `agent: @<你的 id>` + 补 `files`，然后跑 `bash scripts/tasks-index.sh`。
- **合 `develop`**：改 `status: done` + `updated: <日期>`，跑 index。
- **合 `beta` 部署 dev 服**：改 `status: archived` + `updated: <部署日期>`，跑 index。
- **合 `main` 上线 prod**：把任务提炼成 `CHANGELOG.md` 新版本段（Keep a Changelog 格式），`trash docs/tasks/T-XXX.md`，跑 index。
...
```
✅ 任务管理层已升级到 v2.0.0（从 v<INSTALLED_VERSION>）：

  • scripts/tasks-index.sh （已覆盖到最新版）
  • scripts/tasks-new.sh   （已覆盖到最新版）
...
- **Path C 的版本比较**：SKILL.md 声明 `CURRENT_VERSION="2.0.0"`；升级时若发现 `installed_version != CURRENT_VERSION`，直接覆盖脚本 + 重跑 index 即可。不要试图跨版本做数据迁移（v2 内部结构保持向后兼容，只可能扩字段）。
- **分支模型是参数不是常量**：模板里的 `develop` / `beta` / `main` 按项目实际分支替换；没有多级发布的项目直接砍掉「历史归档」段和索引脚本里对应的 `emit_group archived` 那行。
